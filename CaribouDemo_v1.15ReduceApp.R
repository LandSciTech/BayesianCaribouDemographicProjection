...
# usage
packages <- c("shiny", "shinydashboard", "shinyFiles", "shinyjs", "waiter",
              "R2jags","gdata","mcmcplots",
  "ggplot2", "rmarkdown", "knitr",
  "RODBC","plyr","survival","gdata","data.table","caribouMetrics","tidyr","dplyr")
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem(
        ...
        shinyFilesButton("scn_file", "Select File", "Please select a file",
                         multiple = FALSE),
        div(
          id = "nYears",
      ...
      menuItem(
        ...
        sliderInput(inputId = "rQ", label = "Recruitment quantile",
                    value=scn_defaults$rQ, min=0.025, max=0.975),

        sliderInput(inputId = "sQ", label = "Survival quantile",
                    value=scn_defaults$sQ, min=0.025, max=0.975),

        sliderInput(inputId = "rS", label = "Multiplier for effect of disturbance on recruitment",
                    value=scn_defaults$rS, min = 0, max=5),

        sliderInput(inputId = "sS", label = "Multiplier for effect of disturbance on survival",
      ...
      menuItem(
        ...
        div(
          ...
          selectInput("recruitmentModelNumber",
                       ...
                       choices = popGrowthTableJohnsonECCC %>%
                        filter(responseVariable == "recruitment") %>%
                        group_by(ModelNumber) %>%
                        filter(all(Coefficient %in% c("Intercept", "Precision",
    ...
    shinySaveButton("saveOutput", title = "Save results to csv or rds",
...
server <- function(input, output, session) {
  ...
  volumes <- eventReactive(c(input$scn_file, input$saveOutput, input$popGrowth_file),
                           ignoreInit = TRUE,{
    message("Accessing local file structure.",
  ...
  observeEvent(c(input$scn_file, input$saveOutput, input$popGrowth_file), {
    shinyFileChoose(input, "scn_file", roots = volumes(), session = session,
                    filetypes = "csv")

    shinyFileChoose(input, "popGrowth_file", roots = volumes(), session = session,
                    filetypes = "csv")

    shinyFileSave(input, "saveOutput", roots = volumes(), session = session,
                  filetypes = c("csv", "rds"))
  })
  ...
  observeEvent(input$Run.model, updateNavbarPage(inputId = "bodyTabs",
  ...
  dataInput <- eventReactive(input$Run.model, {
    ...
    scns <- expand.grid(P = input$nYearObs, J = input$nYearProj,
    ...
    if(input$nat_model == "default"){
      popGrow_df2 <- popGrowthTableJohnsonECCC
    } else {
      if(is.integer(input$popGrowth_file)){
        popGrow_df2 <- popGrowthTableJohnsonECCC
      } else {
    ...
    oo <- simulateObservations(scns, distScen = scn_df2, cowCounts = cowCounts,
    ...
    betaPriors <- getPriors(modifiers = isolate(reactiveValuesToList(input)),
                            popGrowthTable = popGrow_df2)

    out <- runRMModel(survData = oo$simSurvObs, ageRatio.herd = oo$ageRatioOut,
  ...
  output$table2 <- renderTable({ dataInput2()})
  ...
  output$table3 <- renderTable({ dataInput3()})
  ...
  output$table4 <- renderTable({ dataInput4()})
  ...
  output$table5 <- renderTable({ dataInput5()})
  ...
  output$table7 <- renderTable({ dataInput7()})

  output$table6 <- renderDataTable({

    columnNAMES <- colnames(dataInput()$result$BUGSoutput$summary)

    x<-data.frame(row.names(dataInput()$result$BUGSoutput$summary),
